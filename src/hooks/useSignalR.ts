...
// ─────────────────────────────────────────────────────────────────────────────
//  CONNECTION CONFIG
//  For direct App Runner test, set these two:
//    DIRECT_APP_RUNNER_URL  = your App Runner service URL
//    INTERNAL_SECRET        = value from Secrets Manager / CloudFormation param
//
//  Example:
//    const DIRECT_APP_RUNNER_URL = "https://abc123.ap-south-1.awsapprunner.com";
//    const INTERNAL_SECRET       = "your-internal-routing-secret-value";
//
//  Set DIRECT_APP_RUNNER_URL to "" to use the HUB_URL from constants instead.
// ─────────────────────────────────────────────────────────────────────────────
const DIRECT_APP_RUNNER_URL = "https://x3kkneepfu.ap-south-1.awsapprunner.com";
const INTERNAL_SECRET = "e4f1b7c2a9d3e6f0c4a7b2d9e1f6c8a3d5b7c9e2f1a4d6c8b0e3f7a2c5d9b1e6";

const HUB_ENDPOINT = DIRECT_APP_RUNNER_URL
  ? `${DIRECT_APP_RUNNER_URL}/hubs/support`
  : HUB_URL;
  ...
  useEffect(() => {
    ...
    const conn = new signalR.HubConnectionBuilder()
      .withUrl(HUB_ENDPOINT, {
        // X-Internal-Secret injected here for direct App Runner connection.
        // When going through API Gateway this header is injected by the
        // Lambda authorizer instead — remove headers: {} in that case.
        headers: DIRECT_APP_RUNNER_URL
          ? { "X-Internal-Secret": INTERNAL_SECRET }
          : {},
        // Skip negotiation only when connecting directly to App Runner
        // (avoids an extra round trip). Remove this when using API Gateway.
        skipNegotiation: false,
        transport: DIRECT_APP_RUNNER_URL
    ...
    conn.start()
      ...
      .catch((err) => {
        ...
        setUsers(DEMO_USERS);
        pushEvent({
          id: uid(), type: 'info', userId: 'sys', userName: 'System',
          detail: `Demo mode — server not reachable at ${HUB_ENDPOINT}`, ts: new Date()
        });
      });
